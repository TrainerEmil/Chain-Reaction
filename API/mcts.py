...
[1] O(1) total-visit count in _select (carried over)
    parent.N == sum(child.N) after each completed backprop.

[2] Default-argument lambda binding in _select (carried over)
    Avoids dict lookups in the inner selection loop.

[3] BATCHED LEAF EVALUATION WITH VIRTUAL LOSS  ← new, biggest gain
    ---------------------------------------------------------------
    Problem
    ~~~~~~~
    The previous implementation evaluates leaves one at a time:
        select → expand (1 model call) → backprop
    With mcts_simulations=12 and ~35 moves per game that is
        12 × 35 × 50 games = 21 000 separate forward passes per iteration,
    each on a batch of size 1.  On CPU, per-call overhead (Python
    dispatch, BatchNorm with B=1, memory allocation) often exceeds the
    actual floating-point work.

    Solution
    ~~~~~~~~
    When `inference_batch_size > 1`:

      a) Select up to `inference_batch_size` leaves in one go using PUCT.
         Virtual loss steers concurrent selections to *different* leaves.

      b) All unique unexpanded leaves are encoded into a single NumPy
         array and evaluated with *one* model.forward() call.

      c) Each leaf is expanded with the pre-computed outputs.
         Virtual loss is removed; real values are backpropagated.

    With inference_batch_size=8 and mcts_simulations=12 this cuts model
    calls from 12 down to 2 per move (6× reduction).
