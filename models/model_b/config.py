...
class Config:
    ...
    # ── GNN architecture (CR-EGNN) ─────────────────────────────────────
    # hidden_dim : width of node feature vectors in every message-passing layer.
    # num_layers : number of message-passing rounds (= max chain-reaction depth
    #              the network can explicitly reason about).
    gnn_hidden_dim: int = 128
    gnn_num_layers: int = 6

    # ── CNN architecture (kept for checkpoint compatibility) ────────────
    # These fields are no longer used by the CR-EGNN model but are retained
    # so that old training-state files can still be loaded without KeyErrors.
    num_filters:    int = 64
